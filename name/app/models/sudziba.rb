class Sudziba < ApplicationRecord
	...
	validates :customer, presence: { message: "ir obligati jaievada" }, length:  {maximum: 30}, format: { with: /\A[a-zA-Z]+\z/, message: "var saturet tikai burtus" }
	validates :user, presence: { message: "ir obligati jaievada" }, length:  {maximum: 30}, format: { with: /\A[a-zA-Z]+\z/, message: "var saturet tikai burtus" }
